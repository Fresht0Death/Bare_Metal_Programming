...
void pc13_exti_init(void)
{
	...
	/* Best practice: set PC13 as input */
		...
		// table for how to config as input
	GPIOC->MODER &=~(1U<<26);
	GPIOC->MODER &=~(1U<<27);
	...
	SYSCFG->EXTICR[3] |= (1<<5); // simiarly to our alt-func register we use an array to access the nth EXTICRn by using n-1 based indexing
	...
	NVIC_EnableIRQ(EXTI15_10_IRQn/*passed interrupt here, this is a tough one*/);
