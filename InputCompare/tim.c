...
#define AF6_TIM       		(1U<<25)// what bit must be set enable alt function mode for AF6
#define TIM3EN				(1U<<1)
#define CCER_CC1S     		(1U<<0) // what bit must be set to to enable CC1S?
...
void tim3_pa6_input_capture(void)
{
	...
	/* Enable clock access to GPIOA */
		// how do you enable GPIOA?????
		RCC->AHB1ENR |= GPIOAEN; // enabling again so we can have function act independently

	/* Set PA6 mode to alternate function */
		// how does alternate function mode work?
		// GPIOA mode register look up to enable alt functoin moder for PA6
		GPIOA->MODER &=~ (1U<<12);
		GPIOA->MODER |= (1U<<13);
	...
	/* Set PA6 alternate function type to TIM3_CH1 (AF02) */
		...
		// check how to set to alt func mode
		// go to truth table and not we have to set bit 25 to ???
				// use symbolic name
		GPIOA->AFR[0]|= AF6_TIM;
	...
	/* Look at how we enaled tim2 previously. Should be similar. */
		RCC->APB1ENR |= TIM3EN;


	/* Set prescaler */
	/* what do we set = (this is done in a different way though -> 16 000 000 000 / 16 000 */
	TIM3->PSC = 16000 - 1;
	...
	TIM3->CCER /* Timer 3 capture compare Enable register */ |= CCER_CC1E;/* what is the symolic name for what bit */

	/* Enable TIM3 */
	TIM3->CR1 |= CR1_CEN;
	// look at how we enabled tim2
