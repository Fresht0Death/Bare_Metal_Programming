...
void pa1_adc_interrupt_init(void)
{
	...
	printf("We called the adc_interupt function\n\r");

	//copy code from previous adc init function for pa1
	/*** Configure the ADC GPIOA pin ***/
	/* Enable clock access to gpioA*/
	RCC->AHB1ENR |= GPIOAEN;
	/* Set the mode of PA1 to analog mode which is done in the mode register */
	...
	NVIC_EnableIRQ(ADC_IRQn);		// reguls nvic function with adc irqn as arugement
